...
if(!instance.draw){if(instance.sprite){drawInstance(instance);const nextFrame=instance.frameNumber+instance.animationSpeed;instance.frameNumber=nextFrame<instance.sprite.length-1?nextFrame:0}}else{ctx.save();instance.draw();ctx.restore()}
...
const moveToPoint=(object,x,y,speed)=>{const d=distanceBetween(object.x,object.y,x,y);if(d>speed){const v=getDirectionVector(object.x,object.y,x,y);object.x+=(x-object.x)/d*speed;object.y+=(y-object.y)/d*speed}else{object.x=x;object.y=y}}
...
const checkBoxCircle=(obj1,obj2)=>{const box={x1:obj1.x+obj1.collider.x1,x2:obj1.x+obj1.collider.x2,y1:obj1.y+obj1.collider.y1,y2:obj1.y+obj1.collider.y2};const v1=distanceBetween(box.x1,box.y1,obj2.x,obj2.y);const v2=distanceBetween(box.x1,box.y2,obj2.x,obj2.y);const v3=distanceBetween(box.x2,box.y1,obj2.x,obj2.y);const v4=distanceBetween(box.x2,box.y2,obj2.x,obj2.y);const centerInBox=obj2.x>obj1.x+obj1.collider.x1&&obj2.x<obj1.x+obj1.collider.x2&&obj2.y>obj1.y+obj1.collider.y1&&obj2.y<obj1.y+obj1.collider.y2
...
const drawSprite=(sprite,x,y,rotation=0,scaleX=1,scaleY=1,opacity=1,frameNumber=0,tpX=0,tpY=0)=>{const screenX=x-camera.x-tpX;const screenY=y-camera.y-tpY;if(screenX+sprite[0].width>0&&screenY+sprite[0].height>0&&screenX<canvas.width/camera.scaleX&&screenY<canvas.height/camera.scaleY){ctx.save();ctx.translate(Math.floor((x-camera.x)*camera.scaleX),Math.floor((y-camera.y)*camera.scaleY));ctx.rotate(rotation*Math.PI/180);ctx.scale(scaleX*camera.scaleX,scaleY*camera.scaleY);ctx.globalAlpha=opacity>=0?opacity:0;ctx.drawImage(sprite[Math.floor(frameNumber)],Math.floor(-tpX),Math.floor(-tpY));ctx.restore()}}